...
The former claims of 13–63% faster small messages, 3–4× large-message cost, only 20–30% byte-message overhead and zero framework allocation are withdrawn. The old end-to-end throughput echoed while the TCP control only counted bytes. The separate large-message codec experiment did not echo, but still performed message materialization and other work omitted by the byte-count control; cross-experiment subtraction cannot isolate framework tax. Historical numbers are not necessarily wrong, but workloads differ and auditable raw records were not checked into this repository. Earlier microbenchmark percentages and session-cost summaries are also no longer current promises; rerun their individual classes as needed.
