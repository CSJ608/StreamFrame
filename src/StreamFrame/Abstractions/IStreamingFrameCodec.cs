...
/// </code>
/// 相比 <see cref="IFrameCodec.EncodeFrame"/> 的"负载整体进、帧整体出"，本方式
/// 全程单缓冲、零 memcpy。已写入的字节可通过 <see cref="IWrittenBufferWriter.WrittenSpan"/>
/// 原地回填（如 <see cref="LengthPrefixFrameCodec"/> 的 4 字节长度头）。
///
/// 协议语义（帧边界、超长防御）与对应 <see cref="IFrameCodec"/> 完全一致。
/// </summary>
public interface IStreamingFrameCodec : IFrameCodec
{
